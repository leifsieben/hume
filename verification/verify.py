...
actually featurise.
    ...
    python verification/verify.py                 # benchmark set, quick
    python verification/verify.py --corpus        # full 1M training corpus, the real gate
    python verification/verify.py --corpus -n 50000
...
def _load(source: str, limit: int | None) -> list[str]:
    if source == "corpus":
        ...
        if sel.exists():
            ...
            sys.exit(f"no corpus at {sel} -- run research/corpus.py select first")
    else:
