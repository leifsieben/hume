...
Writes per-model bench predictions so research/downstream.py can measure what the reconstruction
actually buys.
"""
...
def build_gnn(h, depth, t_out):
    ...
    checkpoint written before this refactor still load (research/proxy_cost.py asserts that with
...
def main() -> None:
    ...
    assert Xb.shape[1] == X.shape[1], (
        ...
        f"split changed; re-run research/assemble.py for the benchmark before training")

    if a.contiguous_split:
        # `research/corpus.py select` emits each scaffold's DEPTH molecules adjacently, so a contiguous
