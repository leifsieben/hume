...
each edit inflate its own denominator and pull every model toward the same score.
    ...
    python verification/embed_pairs.py ecfp r3cfp chemberta molformer
    python verification/embed_pairs.py desc          # slow, run it in the background
"""
...
def _all_smiles():
    ...
    The background is READ from background.json, which research/make_pairs.py reserves BEFORE it builds
    ...
    assert not (set(bg) & seen), "background overlaps the pairs; re-run research/make_pairs.py"
