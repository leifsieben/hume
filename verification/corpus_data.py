...
`verification/models.py` was written against a single `train.npz` that fits comfortably in memory at 100k.
...
Three levers, in the order worth reaching for:
  ...
  `shards=`                 hold out whole shards. Shards follow selection order, which is
                            ...
                            one, and `scaffold_split` in verification/models.py remains the real answer.
...
def load_bench(dtype=np.float32):
    ...
    fallback will trip the column-count assertion in `verification/models.py` -- deliberately, since the
