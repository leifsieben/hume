...
`hume.featurize_blocks` no longer calls `rdMolDescriptors._CalcCrippenContribs`: the (logP, MR)
...
checks the thing that is actually shipped -- the header, compiled into hume._core, fed by
...
tolerance could only ever hide a wrong row.
    ...
    .venv/bin/python src/hume/_verify_crippen.py [n_mols] [smiles_file]
"""
