...
list restricted to the surviving rdkit ones.
    ...
    .venv/bin/python bench/bench_e2e.py hume      [n_mols] [n_reps]
    <mordred env>    python bench/bench_e2e.py baseline  [n_mols] [n_reps]
    .venv/bin/python bench/bench_e2e.py report
...
def _survivors_covered(cols) -> int:
    ...
    position by roughly a factor of 1.5, which is the error docs/notes/PORT_STATUS.md's "warning about the
    number 182" is about.
    ...
    HUME's own verification/chi.py did) while the 865 carry RDKit's `Chi0n`..`Chi4v`. Those nine are the same
