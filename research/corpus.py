"""Build the 1M-molecule training corpus.
    ...
    python research/corpus.py scaffolds [--workers 6]     scan the pool, cache scaffold assignments
    python research/corpus.py select    [--n 1000000]     scaffold-stratified pick, benchmark excluded
    python research/corpus.py compute   [--workers 6]     descriptors, sharded and resumable
    python research/corpus.py pack                        slice shards into X / Y and report NaN rates
...
RDKit. `verification/chi.py`, `verification/cycles.py`, `verification/resistance.py`, `verification/conjugation.py` and `verification/stereo.py` are the fast
implementations that will actually run at inference. They are computed here too, side by side,
so `verification/verify.py --corpus` can check one against the other over all 1M molecules. A descriptor is
