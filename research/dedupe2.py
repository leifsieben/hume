"""Deduplicate the FULL descriptor space -- RDKit + Mordred + ours -- under one criterion.
    ...
    python research/dedupe2.py corpus     # build + save the stratified corpus
    python research/dedupe2.py compute    # compute all 2,023 columns on it   (the expensive step)
    python research/dedupe2.py analyse    # correlate, cover, and write the review reports

WHAT THIS FIXES relative to research/dedupe.py, each item measured rather than assumed:
...
2. NO IMPUTATION. research/dedupe.py median-imputed NaN before correlating. 4,099 column pairs share their
...
3. STRATIFIED CORPUS, AND THE MINIMUM ACROSS STRATA. Redundancy is not a property of a corpus
   ...
   measured in research/dedupe_cost.py, where columns dropped at >=0.99 on a ChEMBL prefix turned out to be
...
def pairwise_complete_corr(A, M):
    ...
    defined. research/dedupe.py median-imputed first, and 4,099 pairs share a missingness pattern, so the
