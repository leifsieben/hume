...
index an embedding matrix by position without going through `load_arm`.
    ...
    python research/embed_bench.py index                       # build/refresh the master index
    python research/embed_bench.py embed chemberta_mtr         # one arm (respects --tier)
    python research/embed_bench.py embed molformer --tier 0
    python research/embed_bench.py verify                      # check every arm on disk
...
# --------------------------------------------------------------------------------------------
# arms -- reuse verification/embed_pairs.py's implementations rather than reimplementing them
# --------------------------------------------------------------------------------------------
...
def _arm_molformer(smiles):
    ...
    verification/embed_pairs.py rather than patched there, because Figure A's cached npz files were computed
