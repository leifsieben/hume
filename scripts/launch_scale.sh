...
# the loop: it boots, downloads boot/boot.sh and bundle.tar.gz from S3, runs bench/bench_aws.py for the
...
#    has no AWS CLI -- scripts/boot_scale.sh is where the CLI gets installed -- so the very first command
...
{ printf '#!/bin/bash\nexport ROLE=%s\nexport ONLY_ARMS="%s"\n' "$ROLE" "$ONLY_ARMS"
  tail -n +2 scripts/boot_scale.sh; } | gzip -9 > "$UD_FILE"
...
if [ "$UD_B64" -gt 16384 ]; then
  echo "FATAL: scripts/boot_scale.sh no longer fits in user-data even compressed. Go back to fetching it
