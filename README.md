...
# (2, 2670) float64: 622 `default` descriptors, then 2,048 ECFP bits
...
```python
molhume.featurize(smiles, columns="default")      # 622, the default -- the safest set
molhume.featurize(smiles, columns="small")        # 408 -- free, and the cheapest to compute
molhume.featurize(smiles, columns="minimal")      # 256 -- a stated trade, not a free cut
...
| --- | ---: | ---: | ---: |
| `default` | 622 | **−1.17%, p = 0.001** | −1.12% |
| `small` | 408 | +0.30%, p = 0.711 | −0.07% |
...
| --- | --- | --- |
| `columns` | `"default"` | `"default"` (622), `"small"` (408), `"minimal"` (256), `"full_no_new"` (1,109), `"full"` (1,269), a versioned name, or a list of names in the order wanted. Decides what is computed as well as what is returned |
