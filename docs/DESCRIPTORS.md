...
That union is heavily redundant, so it is deduplicated (`research/dedupe.py` → `data/dedupe.json`).
...
that are redundant *there* can separate on a specific endpoint. Measured (`research/dedupe_cost.py`)
...
that caused the drop. `research/dedupe_loo.py` adds one dropped column back at a time to see whether the
...
(`docs/notes/PORT_STATUS.md:499`). The re-run should close this; until it does, cite the kept set as
...
The house rule (`docs/notes/PORT_STATUS.md` §1):
...
| 2 | `[SeD2H0]` and its Li/Be/Si/Ge/As/Sn/Pb equivalents parse to element-*number* queries with no aromaticity constraint, so `aaSe` fires on selenophene's aromatic selenium. The same quirk recurs in the SMARTS compiler, where only the organic subset gets `AtomType`. | `estate_typer.h:24`, `docs/notes/PORT_STATUS.md:785` | `ssSe`, `aaSe`, QED alert 26 |
...
> **`docs/notes/PORT_STATUS.md:133` says "two have already been found and kept". That sentence is a summary
...
> `docs/notes/PORT_STATUS.md:379` — that block is stale and still says the columns are unwired.
...
`research/block_run.py` ran the intended test — each block added to an `ecfp+core` baseline over 34
...
* `FINDINGS.md:189` lists cyclic homomorphism counts as "held in reserve — deliberately not
  built". They were built; `verification/cycles.py` and `hume_blocks.h:471` are shipping.
* `METHODS.tex:4` still describes selection as "a run of 20,000 ChEMBL molecules" and the
  reproduced defects as "three".
* `docs/notes/PORT_STATUS.md:379` still says `Ipc`/`AvgIpc`/`Log2Ipc` "stay unwired"; they ship.
