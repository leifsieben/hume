...
The intuition that this would be ~2/3 RDKit is wrong in count and right in proportion: almost
...
**A "99.9% of variance at 300 components" claim is a preprocessing artifact.** In raw space
...
**AND LINEAR VARIANCE IS THE WRONG CRITERION ANYWAY.** This project has already paid for that
...
### Three things Stage 1 CANNOT establish, and one it actively gets wrong
...
**This is also the case for the two-stage method.** Stage 1 alone ranked `fr_*` worst of any
...
**Two datasets dominate the arithmetic MEAN and it should not be quoted.** `hia` (−17.15%) and
...
**THE ORDERING PRINCIPLE IS NOT ESTABLISHED, AND I EXPECTED IT TO BE.** `_lo` never beats
...
**INDIVIDUALLY FREE DOES NOT COMPOSE.** Applying all eight reductions at once removes 172
...
2. **WHICH COLUMNS ARE DROPPED MATTERS, DECISIVELY.** A *random* 418 costs a median 1.44% and
...
This was not the hypothesis. §7 was written expecting low orders and short lags to carry the
...
### The cost is almost entirely in classification
...
**And five of the original 33 were chempfn LOCKED sets** — `herg` and the four `wong_*` —
...
**One control in this run was degenerate and its p-value means nothing.** `random411` drew 411
...
**FIGURE C'S 33 DATASETS ARE SPENT FOR THIS SPEC.** Every selection round used all of them.
