...
| `descriptors` selects a mode | `columns`: `"default"` / `"small"` / `"minimal"` / `"full_no_new"` / `"full"`, a versioned name, or a list of names | this file split one parameter into two (`additional_descriptors` plus `columns`) on the argument that *whose* descriptors and *which* ones are orthogonal questions. They are, and it was still the wrong shape: every real call answered both at once, the AND between them needed a warning to be comprehensible, and the common case — "the reduced set" — could only be said by passing 622 strings. 0.7.0 collapsed them back into one parameter with named answers and an escape hatch (three then, five from 1.0.0), and **this file's original single-parameter instinct was closer to right than the revision.** |
...
needs no explanation. The internal `core` / `predict` / `blocks` terms stay inside `verification/blocks.py`;
