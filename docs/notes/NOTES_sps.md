...
Deliverables: `src/hume_core/sps.h`, `verification/verify_sps.py`, this file. `src/hume_core/bindings.cpp` was
...
call, after parsing — which is what `_extract.py` does, and what `verification/verify_sps.py` now does.
...
1. **The ring set is the repaired one, not RDKit's raw `GetSymmSSSR`.** `findPotentialStereo`
   ...
   `verification/verify_sps.py --rings repaired` and `--rings rdkit` each give **20,000 / 20,000**. The
...
`verification/verify_sps.py` (test harness only — it computes no descriptor value; it calls RDKit for ground
...
`hume._core`, because `bindings.cpp` is off limits while five agents share this checkout.
    ...
    .venv/bin/python verification/verify_sps.py
...
Measured by `verification/verify_sps.py` — minimum over 9 repetitions per molecule, both arms in the same run
