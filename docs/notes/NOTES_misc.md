...
`src/hume_core/misc_ext.h` · `verification/verify_misc.py` · build dir `build_misc/`
...
`.venv/bin/python verification/verify_misc.py` — a test harness only; nothing in it computes a descriptor.
...
Regenerate those shards with the harness itself — the generator is embedded in `verification/verify_misc.py`
and written out to `build_misc/md_ref.py`, so there is no fourth file:

```
.venv/bin/python verification/verify_misc.py --gen-md-ref
```
...
All numbers below are from `.venv/bin/python verification/verify_misc.py --reps 3` over the whole
...
Statistic: **best of 3 per molecule**, from `verification/verify_misc.py --reps 3`. The corpus was measured
...
```
.venv/bin/python verification/verify_misc.py                 # all 20,000, grades everything
.venv/bin/python verification/verify_misc.py --n 2000        # the smallest stratum, ~30 s
.venv/bin/python verification/verify_misc.py --reps 3        # with timing
.venv/bin/python verification/verify_misc.py --only BertzCT  # one column
```
