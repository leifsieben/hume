...
(`cpp/verify_crippen.py:194`), and `research/assemble.py` builds every matrix from
...
```
pyproject.toml
src/hume/__init__.py         featurize(), the public API in docs/API.md
...
`constraints.txt` and `docs/API.md`).

Deliberately NOT in milestone 1: the other 683 columns (most have no C++ at all), the surrogate,
wheels, and the public `featurize()` signature from `docs/API.md`. Those come after the bridge exists.
