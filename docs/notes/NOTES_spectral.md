...
`src/hume_core/spectral.h`, verified by `verification/verify_spectral.py`.

    cmake -S . -B build_spectral -DCMAKE_BUILD_TYPE=Release && cmake --build build_spectral -j4
    .venv/bin/python verification/verify_spectral.py             # grade against both references + time
    .venv/bin/python verification/verify_spectral.py --shuffle   # + the atom-renumbering probe
    .venv/bin/python verification/verify_spectral.py --no-ref    # skip the float64 regeneration

The cmake line builds the extension as it stands; `spectral.h` is **not wired into
`bindings.cpp`** (contract: five agents, one enum). `verification/verify_spectral.py` compiles its own
...
* a **float64 regeneration** from mordred 1.2.0 on the same 20,000 SMILES for exactly these 65
  columns (`verification/verify_spectral.py --ref`, cached in the scratchpad) -- this one carries the grade.
...
`verification/verify_spectral.py --shuffle` recomputes the whole corpus under four random atom relabellings
...
`verification/verify_spectral.py` supplies the real value from RDKit, so both columns **are** graded above --
...
* **The guard in `leading_vector` fires**, 62 times over the corpus on 59 molecules, and this
  ...
  five divergent molecules. `verification/verify_spectral.py` prints the count on every run.
