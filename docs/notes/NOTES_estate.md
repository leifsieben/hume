...
`src/hume_core/estate_ext.h`, verified by `verification/verify_estate.py` over all 20,000 molecules of
...
`verification/verify_estate.py` therefore runs **live mordred** (`.venv-mordred/bin/python`, only the 22
...
`verification/verify_estate.py` exits non-zero on this and prints `NOT FULLY EXACT`.
...
`verification/verify_estate.py` carries the corrected order as a **diagnostic** (`drv <in> <out> rdkitorder`,
...
`verification/verify_estate.py` prints, per column, the molecules with `N<t> == 0` from the reference matrix
...
## Reproducing
    ...
    .venv/bin/python verification/verify_estate.py          # dump, build the driver, grade x3, check, time
    .venv/bin/python verification/verify_estate.py --keep   # keep build_estate/*.txt and the mordred JSON
