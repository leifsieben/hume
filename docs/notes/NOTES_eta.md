...
Deliverables: `src/hume_core/eta.h`, `verification/verify_eta.py`, this file. `bindings.cpp` untouched; the
...
`.venv/bin/python verification/verify_eta.py` — all 20,000 molecules of `data/dedupe2/corpus.json`, graded
...
guarantee, and `verification/verify_eta.py --invariance` shows the real exposure: renumbering the atoms of
...
```
.venv/bin/python verification/verify_eta.py                # 20,000 molecules, ~3 min the first time
.venv/bin/python verification/verify_eta.py --invariance   # plus the renumbering / Kekule screen
.venv/bin/python verification/verify_eta.py --limit 300    # quick
```

`verification/verify_eta.py` computes no descriptor value. It parses the SMILES, marshals
