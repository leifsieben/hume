...
Harnesses: `research/make_pairs.py` (Fig A pairs) · `verification/embed_pairs.py` (Fig A embeddings) ·
`research/figa_resolution.py` (Fig A metric) · `bench/bench_downstream.py` (Fig B/C grid) ·
`bench/bench_aws.py` + `research/bench_scale_e2e.py` (Fig D throughput) · `bench/collect_scale.py` (Fig D collection).
...
assigned. Asserted in `research/figa_resolution.py`, not assumed. *Found by the CLIMB figures session
...
**Inputs.** Benchmark chemotypes → `research/make_pairs.py` → 15,000 pairs across 15 conditions;
...
* *Protocol beats timestamp.* Every record carries the harness version that produced it
  (`PROTO` in `bench/bench_downstream.py`) and a newer protocol wins however recently the older file
...
`bench/bench_downstream.py`): 3 bases (`ecfp`, `desc`, `ecfp_all_desc`) × 4 embeddings
...
* `minimol` failed to build (`torch-sparse` imports torch in its own setup.py without declaring
  ...
  is a requested arm.
* The bundle carried the ChemBERTa weights at its ROOT while `verification/embed_pairs.py` looks under
...
`feature_weights` (docs/API.md §7): fingerprint bits get weight `w`, descriptors 1, and `w` is tuned
...
- [ ] Run `research/figa_resolution.py` over all arms
