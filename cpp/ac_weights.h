// The nine Autocorrelation weight vectors, in C++. Transliterated from mordred 1.2.0
...
// `ap.getters[w](atom)` nine times per atom -- 473.9 us/mol, the single largest item in the HUME
// pipeline, larger than every other block combined -- and hand the C++ a text file of finished
...
//
// THE NINE, exactly as mordred defines them:
//
...
//        intrinsic-state getter is short `s`, and `s` is not one of these nine.
...
static const int NW = 9;  // c d dv i p v se pe are -- this order is fixed by verify_ac.py
...
// One atom as it comes off the wire. Isotope is deliberately absent: all nine getters read
